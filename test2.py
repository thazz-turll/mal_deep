#!/usr/bin/env python3
"""
MalGAN training script (PyTorch) - Corrected Version

- Fixes data leakage by using pre-split train/val sets.
- Trains G and S only on (X_train, y_train).
- Evaluates evasion rate only on (X_val, y_val).
- Fixes blackbox query (removes binarization).
- Saves the Generator with the best validation evasion_rate.
"""
import os
import numpy as np
import torch
...
# Model save paths
GEN_SAVE_PATH = "generator_malgan_best.pt"
SUB_SAVE_PATH = "substitute_detector.pt"
...
def query_blackbox_from_tensor(tensor_batch):
    ...
    # arr_bin = (arr > 0.5).astype(int)  <--- ĐÃ XÓA BƯỚC NÀY
...
# ================== Training loop ==================
print("Start training MalGAN procedure...")
